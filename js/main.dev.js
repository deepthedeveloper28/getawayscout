...
function initGetawayscout() {
  ...
  // 2. Setup SEO, Navigation, Interactivity and Dynamic Branding
  applyPageSEO();
  applySiteBranding();
  setupMobileNav();
  highlightActiveNav();
  ...
  // 3. Setup Cookie Consent & Privacy Preferences
  setupCookieConsent();

  // 4. Setup Instant Page Prefetching & Service Worker Cache for 0ms transitions
  setupInstantPagePrefetch();
  registerServiceWorker();

  // 5. Setup Lazy Third-Party Affiliate Tracking (Travelpayouts, GYG, Expedia)
  lazyLoadThirdPartyTracking();

  // 6. Setup Home Search Tabs
  setupHomeSearchTabs();

  // 7. Inject Page Specific Widgets
  injectPageWidgets();
  initExpediaWidgets();

  // 7. Render 3 Random Live WordPress Blogs on Homepage (Ultra-fast 0ms cache + fresh live fetch)
  if (typeof VoyageurWP !== 'undefined' && typeof VoyageurWP.renderHomeBlogs === 'function') {
    VoyageurWP.renderHomeBlogs('homeBlogsGrid', 3);
  }

  // 8. Background Prefetch WordPress & WooCommerce data for 0ms instant transitions
...
/**
 * Automatically applies master SEO configurations (Title, Description, Keywords, OpenGraph)
 * from affiliateConfig.seo across any current page.
 */
...
/**
 * ==============================================================================
 * 🍪 COOKIE CONSENT & PRIVACY PREFERENCES SYSTEM (GDPR / CCPA / PERFORMANCE)
 * ==============================================================================
 */
...
function setupCookieConsent() {
  ...
  // Create and inject Banner and Modal HTML
  const bannerHtml = `
  ...
  // Show banner with smooth entrance animation
  setTimeout(() => {
...
/**
 * ==============================================================================
 * ⚡ INSTANT LINK PRE-FETCHING (0ms Ultra-Fast Page Transitions)
 * ==============================================================================
 */
function setupInstantPagePrefetch() {
  ...
  // Monitor hover and touch on internal navigation links
...
/**
 * ==============================================================================
 * 🚀 SERVICE WORKER REGISTRATION (Cache Storage & Offline Resilience)
 * ==============================================================================
 */
...
/**
 * ==============================================================================
 * 🎯 LAZY THIRD-PARTY AFFILIATE & TRACKING LOADER (Zero Speed Penalty)
 * Automatically loads external scripts (Travelpayouts tp-em.com, GetYourGuide,
 * Expedia) on first user interaction or idle callback.
 * ==============================================================================
 */
function lazyLoadThirdPartyTracking() {
  ...
  function loadExternalAffiliateScripts() {
    ...
    // 1. Remove user interaction listeners
    ...
    // 2. Dynamically inject Travelpayouts Emerald Tracking Script
    ...
    // 3. Dynamically inject GetYourGuide Widget script if page contains a GYG element
  ...
  // Trigger on first user interaction with passive listeners
  ['scroll', 'touchstart', 'mousemove', 'keydown', 'click'].forEach(evt => {
    window.addEventListener(evt, loadExternalAffiliateScripts, { passive: true, once: true });
  });

  // Fallback: Trigger automatically when main thread is idle (or after 2.5s)
